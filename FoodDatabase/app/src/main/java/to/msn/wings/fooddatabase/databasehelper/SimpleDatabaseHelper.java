package to.msn.wings.fooddatabase.databasehelper;
...
import to.msn.wings.fooddatabase.basicdata.MapOpenDay;
import to.msn.wings.fooddatabase.basicdata.MapSearch;
import to.msn.wings.fooddatabase.R;
...
public class SimpleDatabaseHelper extends SQLiteOpenHelper {
    ...
    public void CreateMapMenuTable(SQLiteDatabase db){
        db.execSQL("CREATE TABLE shopmapview (" +
                "id INTEGER PRIMARY KEY, name TEXT , name_kana TEXT, address TEXT, tel TEXT, opentime TEXT, holiday TEXT, image TEXT, favorite INTEGER)");
        ...
        try {
            ...
            try {
                SQLiteStatement sql = db.compileStatement(
                        "INSERT INTO shopmapview(id, name,name_kana,address,tel,opentime,holiday,image, favorite) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)");
                for (int i = 0; i < jsonMapList.size(); i++) {
                    sql.bindLong(1, jsonMapList.get(i).getId());
                    sql.bindString(2, jsonMapList.get(i).getName() );
                    sql.bindString(3, jsonMapList.get(i).getNameKana());
                    sql.bindString(4, jsonMapList.get(i).getAddress());
                    sql.bindString(5, jsonMapList.get(i).getTel());
                    sql.bindString(6, jsonMapList.get(i).getOpentime());
                    sql.bindString(7, jsonMapList.get(i).getHoliiday());
                    sql.bindString(8, jsonMapList.get(i).getImage());
                    sql.bindLong(9, jsonMapList.get(i).getFavorite());
                    sql.executeInsert();
                }
    ...
    public void CreateMapSearchTable(SQLiteDatabase db){
        ...
        db.execSQL("CREATE TABLE shopsearch (" +
                "name TEXT , name_kana TEXT, address TEXT, opentime TEXT, holiday TEXT," +
