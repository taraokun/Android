...
import to.msn.wings.fooddatabase.dao.ShopMapViewDao;
 ...
 * SelectSheetTable
 */
public class MainActivity extends AppCompatActivity implements View.OnFocusChangeListener, SearchView.OnQueryTextListener {
    ...
    public boolean onQueryTextSubmit(String query) {
        ...
        // ヘッダー：個数
        TextView headerNumber = setTextItem("電話番号", GCH);
        TableRow.LayoutParams paramsNumber = setParams(0.35f);
        // ヘッダー：単価
        TextView headerImage = setTextItem("画像", GCH);
        TableRow.LayoutParams paramsImage = setParams(0.25f);
       ...
       // String column = "product";          //検索対象のカラム名
        ...
        for(MapData search_data: all_search_list) {
            ...
            // 個数
            TextView textNumber = setTextItem(search_data.getTel(), GE);      // TextViewのカスタマイズ処理
            // 単価
            TextView textPrice = setTextItem(search_data.getImage(), GE);      // TextViewのカスタマイズ処理
